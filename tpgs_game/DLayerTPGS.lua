...
-----------------------------------------
-- 类定义
CDLayerTPGS = class("CDLayerTPGS", CDTKDTableView)
CDLayerTPGS.__index = CDLayerTPGS

-- 构造函数
function CDLayerTPGS:ctor()
    cclog("CDLayerTPGS::ctor")
    CDLayerTPGS.super.ctor(self)
    CDLayerTPGS.initialMember(self)
...
function CDLayerTPGS:onEnter( ... )

	cclog("CDLayerTPGS::onEnter")
	-- dtOpenWaiting(self)

end

function CDLayerTPGS:onExit()
    cclog("CDLayerTPGS::onExit")

    self:stopAllActions()
    self:enableTouch(false)
    --模拟析构自身
    CDLayerTPGS.releaseMember(self)
...
-- 设置点击标志的开启和关闭
function CDLayerTPGS:enableTouch( bEnable)
	cclog("CDLayerTPGS::enableTouch")
    print("===================")
	...
	if bEnable then
        ...
        local function onTouchBegan(touch, event)
            cclog("CDLayerTPGS::onTouchBegan")
            print("self",self)
        ...
        local function onTouchMoved(touch, event)
            --cclog("CDLayerTPGS::onTouchMoved")
        end
    
        local function onTouchEnded(touch, event)
            cclog("CDLayerTPGS::onTouchEnded")
...
CDLayerTPGS.m_pSimpleTouch = nil         -- 简单场按钮
CDLayerTPGS.m_pNormalTouch = nil         -- 普通场按钮
CDLayerTPGS.m_pHardTouch   = nil         -- 困难场按钮
CDLayerTPGS.m_pChallengeTouch = nil      -- 挑战场按钮


CDLayerTPGS.m_pSimpleGoldNode = nil      -- 简单场金币特效图标挂载点
CDLayerTPGS.m_pNormalGoldNode = nil      -- 普通场金币特效图标挂载点
CDLayerTPGS.m_pHardGoldNode   = nil      -- 困难场金币特效图标挂载点
CDLayerTPGS.m_pChallengeGoldNode =nil    -- 挑战场金币特效图标挂载点


CDLayerTPGS.m_pSimpleGoldLabel = nil     -- 简单场金币消耗文本显示框
CDLayerTPGS.m_pNormalGoldLabel = nil     -- 普通场金币消耗文本显示框
CDLayerTPGS.m_pHardGoldLabel   = nil     -- 困难场金币消耗文本显示框
CDLayerTPGS.m_pChallengeGoldLabel = nil  -- 挑战场金币消耗文本显示框


CDLayerTPGS.m_pGroupGame = nil

-- 初始化
function CDLayerTPGS:initialMember()
    cclog("CDLayerTPGS::initialMember")
...
function CDLayerTPGS:releaseMember()
    cclog("CDLayerTPGS::releaseMember")
...
-- 初始化
function CDLayerTPGS:init()
    cclog("CDLayerTPGS::init")
    self:setVisible(false)
    return true
end

-- 开启界面
function CDLayerTPGS:open()
    cclog("CDLayerTPGS::open")
    ...
    -- self:runAction( cc.Sequence:create( cc.DelayTime:create( 0.3), cc.CallFunc:create( CDLayerTPGS.createRedTask)))
...
-- 添加所有节点
function CDLayerTPGS:refreshInterface()
    cclog("CDLayerTPGS::refreshInterface")
...
function CDLayerTPGS:joinJYDSTable()
...
-- 关闭界面
function CDLayerTPGS:close()
    cclog("CDLayerTPGS::close")
...
-- 关闭界面
function CDLayerTPGS:onClose()
    cclog("CDLayerTPGS::onClose")
...
-----------------------------------------
-- 网络相关处理
function CDLayerTPGS:Handle_Act_Ack( __event)
...
-----------------------------------------
-- ccb处理
function CDLayerTPGS:onAssignCCBMemberVariable(loader)
    cclog("CDLayerTPGS::onAssignCCBMemberVariable")
...
function CDLayerTPGS:onResolveCCBCCControlSelector(loader)
    cclog("CDLayerTPGS::onResolveCCBCCControlSelector")
    loader["onClose"] = function() self:onClose() end
end

-----------------------------------------
-- create
function CDLayerTPGS.createCDLayer_TPGS(pParent)
    cclog("CDLayerTPGS::createCDLayerTPGS")
    if  not pParent then
        return nil
    end
    local layer = CDLayerTPGS.new()
    layer:init()
    pParent:addChild( layer)
    return layer
end

function CDLayerTPGS:onLoadUI()
