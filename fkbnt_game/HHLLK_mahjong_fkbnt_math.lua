...
CDMahjongHHLLKFKBNTMath.m_bIsStopTimer = false             -- 定时器是否停止
CDMahjongHHLLKFKBNTMath.m_nRoundCount   = 0                -- 记录轮次
CDMahjongHHLLKFKBNTMath.m_RandPosArr    = {}               -- 随机位置
CDMahjongHHLLKFKBNTMath.m_nRandCount    = 1                -- 随机的次数
----------------------------------------------------------------------------
-- 宏定义
...
----------------------------------------------------------------------------
--@param _needNumber 需要随机的数量
function CDMahjongHHLLKFKBNTMath:randPos(_needNumber)
    ...
    local nPosRow  = 0
    local isSame =  false
    local compareValue = self.m_nRandCount+_needNumber-1
    while self.m_nRandCount <= compareValue do
        if not self.m_RandPosArr[self.m_nRandCount] then
            self.m_RandPosArr[self.m_nRandCount] = {}
        end

        nPosCol = math.random(1,DEF_HHLLK_FKBNY_MATH_COLUMN)
        nPosRow = math.random(1,DEF_HHLLK_FKBNY_MATH_ROW)
      
        --需要判断一下，如果存在相同的，再次随机

        --检测该位置是否已经有格子占有
        if  not self.m_arrayHHLLKCheckerBoard[nPosCol][nPosRow].isVisible then
           
            for i,v in ipairs(self.m_RandPosArr) do
                if v[1] == nPosCol and v[2] == nPosRow then
                    isSame = true
                    break 
                end
            end
            print("self.m_nRandCount------->",self.m_nRandCount)
            if isSame then
                self.m_RandPosArr[self.m_nRandCount] = {}
            else
                table.insert(self.m_RandPosArr[self.m_nRandCount],nPosCol)
                table.insert(self.m_RandPosArr[self.m_nRandCount],nPosRow)
                self.m_nRandCount = self.m_nRandCount +1
            end
            isSame = false
        end
    end
...
----------------------------------------------------------------------------
function CDMahjongHHLLKFKBNTMath:getRandPosArr()
    return self.m_RandPosArr
end

function CDMahjongHHLLKFKBNTMath:clearRandPosAndCount()
    self.m_RandPosArr = {}
    self.m_nRandCount = 1
end
----------------------------------------------------------------------------
function CDMahjongHHLLKFKBNTMath:removeArrByIndex(index)
    if TABLE_SIZE(self.m_RandPosArr) == 0 or index > TABLE_SIZE(self.m_RandPosArr) then
        return
    end

    table.remove(self.m_RandPosArr,index)
    self.m_nRandCount = self.m_nRandCount-1
end


----------------------------------------------------------------------------
function CDMahjongHHLLKFKBNTMath:setTimerIsStop(boolen)
    self.m_bIsStopTimer = boolen
end

function CDMahjongHHLLKFKBNTMath:getTimerIsStop()
    return self.m_bIsStopTimer
end
----------------------------------------------------------------------------
function CDMahjongHHLLKFKBNTMath:addRoundCount()
    self.m_nRoundCount = self.m_nRoundCount +1
end

function CDMahjongHHLLKFKBNTMath:setRoundCount(num)
    self.m_nRoundCount = num
end

function CDMahjongHHLLKFKBNTMath:getRoundCount()
    return self.m_nRoundCount
end
...
function CDMahjongHHLLKFKBNTMath:checkIsPlaceGrid(_gridPos)
    for i, v in ipairs( _gridPos ) do
        ...
        -- 检测改位置是否有障碍物
        if self.m_arrayHHLLKCheckerBoard[v.y][v.x].isBarrier then
            return false
        end
    end
