...
-----------------------------------------
-- 类定义
CDLayerFKBNT = class("CDLayerFKBNT", CDTKDTableView)
CDLayerFKBNT.__index = CDLayerFKBNT

-- 构造函数
function CDLayerFKBNT:ctor()
    cclog("CDLayerFKBNT::ctor")
    CDLayerFKBNT.super.ctor(self)
    CDLayerFKBNT.initialMember(self)
...
function CDLayerFKBNT:onEnter( ... )

	cclog("CDLayerFKBNT::onEnter")
	-- dtOpenWaiting(self)

end

function CDLayerFKBNT:onExit()
    cclog("CDLayerFKBNT::onExit")

    self:stopAllActions()
    self:enableTouch(false)
    --模拟析构自身
    CDLayerFKBNT.releaseMember(self)
...
-- 设置点击标志的开启和关闭
function CDLayerFKBNT:enableTouch( bEnable)
	cclog("CDLayerFKBNT::enableTouch")
    print("===================")
	...
	if bEnable then
        ...
        local function onTouchBegan(touch, event)
            cclog("CDLayerFKBNT::onTouchBegan")
            print("self",self)
        ...
        local function onTouchMoved(touch, event)
            --cclog("CDLayerFKBNT::onTouchMoved")
        end
    
        local function onTouchEnded(touch, event)
            cclog("CDLayerFKBNT::onTouchEnded")
...
CDLayerFKBNT.m_pSimpleTouch = nil         -- 简单场按钮
CDLayerFKBNT.m_pNormalTouch = nil         -- 普通场按钮
CDLayerFKBNT.m_pHardTouch   = nil         -- 困难场按钮
CDLayerFKBNT.m_pChallengeTouch = nil      -- 挑战场按钮


CDLayerFKBNT.m_pSimpleGoldNode = nil      -- 简单场金币特效图标挂载点
CDLayerFKBNT.m_pNormalGoldNode = nil      -- 普通场金币特效图标挂载点
CDLayerFKBNT.m_pHardGoldNode   = nil      -- 困难场金币特效图标挂载点
CDLayerFKBNT.m_pChallengeGoldNode =nil    -- 挑战场金币特效图标挂载点


CDLayerFKBNT.m_pSimpleGoldLabel = nil     -- 简单场金币消耗文本显示框
CDLayerFKBNT.m_pNormalGoldLabel = nil     -- 普通场金币消耗文本显示框
CDLayerFKBNT.m_pHardGoldLabel   = nil     -- 困难场金币消耗文本显示框
CDLayerFKBNT.m_pChallengeGoldLabel = nil  -- 挑战场金币消耗文本显示框


CDLayerFKBNT.m_pGroupGame = nil

-- 初始化
function CDLayerFKBNT:initialMember()
    cclog("CDLayerFKBNT::initialMember")
...
function CDLayerFKBNT:releaseMember()
    cclog("CDLayerFKBNT::releaseMember")
...
-- 初始化
function CDLayerFKBNT:init()
    cclog("CDLayerFKBNT::init")
    self:setVisible(false)
    return true
end

-- 开启界面
function CDLayerFKBNT:open()
    cclog("CDLayerFKBNT::open")
    ...
    -- self:runAction( cc.Sequence:create( cc.DelayTime:create( 0.3), cc.CallFunc:create( CDLayerFKBNT.createRedTask)))
...
-- 添加所有节点
function CDLayerFKBNT:refreshInterface()
    cclog("CDLayerFKBNT::refreshInterface")
...
function CDLayerFKBNT:joinJYDSTable()
    local function onCallJoinFuc()
        ...
        --     tipStr = string.format("确定花费%d%s开始麻将连连看游戏吗？", self.m_nNeedGold, "金币")
            ...
            g_pSceneHall:goToScenneTable_FKBNT(self.m_nFlag)
...
-- 关闭界面
function CDLayerFKBNT:close()
    cclog("CDLayerFKBNT::close")
...
-- 关闭界面
function CDLayerFKBNT:onClose()
    cclog("CDLayerFKBNT::onClose")
...
-----------------------------------------
-- 网络相关处理
function CDLayerFKBNT:Handle_Act_Ack( __event)
...
-----------------------------------------
-- ccb处理
function CDLayerFKBNT:onAssignCCBMemberVariable(loader)
    cclog("CDLayerFKBNT::onAssignCCBMemberVariable")
...
function CDLayerFKBNT:onResolveCCBCCControlSelector(loader)
    cclog("CDLayerFKBNT::onResolveCCBCCControlSelector")
    loader["onClose"] = function() self:onClose() end
end

-----------------------------------------
-- create
function CDLayerFKBNT.createCDLayer_FKBNT(pParent)
    cclog("CDLayerFKBNT::createCDLayerFKBNT")
    if  not pParent then
        return nil
    end
    local layer = CDLayerFKBNT.new()
    layer:init()
    pParent:addChild( layer)
    return layer
end

function CDLayerFKBNT:onLoadUI()
    ...
    local node  = CCBReaderLoad("CDLayer_FKBNT.ccbi",proxy,loader)
