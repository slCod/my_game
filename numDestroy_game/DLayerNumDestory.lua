...
require( "numDestroy_game.numDestory_math")
require( "numDestroy_game.DItemNum")
require( "numDestroy_game.GlobalData_Num")

require( "numDestroy_game.mahjong_numDestroy")
...
function CDLayerNumDestory:initialMember()
    ...
    self.m_pLabelNumDestroy = nil       -- 点击数字加1的label
    self.m_pLabelMahDestroy = nil       -- 点击麻将加1的label
...
----------------------------------------------------------------------------
-- 创建用户界面
function CDLayerNumDestory:createUserInterface(data)
    cclog("CDLayerNumDestory::createUserInterface")

    self.m_nFlag   = data

    if self.m_nFlag == 1 then
        self.m_pLabelNumDestroy:setVisible(true)
        self.m_pLabelMahDestroy:setVisible(false)
    else
        self.m_pLabelNumDestroy:setVisible(false)
        self.m_pLabelMahDestroy:setVisible(true)
    end
    ...
    for i = 1,self.m_nMaxItemCount do
        local curItemNum = nil
        if self.m_nFlag == 1 then
            curItemNum = CDItemNum.createCDItemNum(self.clip)
        else
            curItemNum = CDMahjong_NUM.createCDMahjong_NUM(self.clip)
...
function CDLayerNumDestory:touchItemNumFromPoint( point)
    ...
    for i = 1, self.m_nMaxItemCount do
        if  self.m_sItemArr[i] and self.m_sItemArr[i]:isVisible() and 
            ...
            self.m_pNumGameMath:onClickItem(touch_h,touch_v,self.m_nFlag)
            ...
            if  TABLE_SIZE(destoryArr) >= 3 then
                ...
                self:refreshChangeSprite()
                self.m_pNumGameMath:checkDestoryItem(self.m_nFlag,destoryArr,touch_h,touch_v)
...
function CDLayerNumDestory:moveToPosInGame( _table )
    ...
    local curNeedRefreshArr = self.m_pNumGameMath:setGroupItemAndChange(self.m_nFlag)
    ...
    local function canPlayGame(  )
        ...
        if  TABLE_SIZE(destoryArr) >= 3 then
            ...
            self.m_pNumGameMath:addLeftChange()
            self:refreshChangeSprite()
            self.m_pNumGameMath:checkDestoryItem(self.m_nFlag,destoryArr)
            local allCount = TABLE_SIZE(self.m_pNumGameMath:getChangeArr())
            local waitTime = 0.01*allCount+0.7
            self:showDestoryScore(self.m_pNumGameMath:getChangeArr())
            self:runAction(cc.Sequence:create( cc.DelayTime:create(waitTime), cc.CallFunc:create( CDLayerNumDestory.moveToPosInGame)))
...
function CDLayerNumDestory:onStartPlay( ... )
    ...
    self.m_pNumGameMath:onResetItemPos(self.m_nFlag)
...
function CDLayerNumDestory:onSetting()
    ...
    self.m_pBtnCloseYX:setVisible(g_pGlobalManagement:isEnableSound())
...
function CDLayerNumDestory:onAssignCCBMemberVariable(loader)
    ...
    self.m_pLabelNumDestroy = loader["label_numDestroy"]
    self.m_pLabelMahDestroy = loader["label_mahDestroy"]
    ...
    self.m_pBtnCloseYX      = loader["btn_closeYX"]  
