...
-----------------------------------------
-- 类定义
CDLayerSZXXL = class("CDLayerSZXXL", CDTKDTableView)
CDLayerSZXXL.__index = CDLayerSZXXL

-- 构造函数
function CDLayerSZXXL:ctor()
    cclog("CDLayerSZXXL::ctor")
    CDLayerSZXXL.super.ctor(self)
    CDLayerSZXXL.initialMember(self)
...
function CDLayerSZXXL:onEnter( ... )

	cclog("CDLayerSZXXL::onEnter")
	-- dtOpenWaiting(self)

end

function CDLayerSZXXL:onExit()
    cclog("CDLayerSZXXL::onExit")

    self:stopAllActions()
    self:enableTouch(false)
    --模拟析构自身
    CDLayerSZXXL.releaseMember(self)
...
-- 设置点击标志的开启和关闭
function CDLayerSZXXL:enableTouch( bEnable)
	cclog("CDLayerSZXXL::enableTouch")
    print("===================")
	...
	if bEnable then
        ...
        local function onTouchBegan(touch, event)
            cclog("CDLayerSZXXL::onTouchBegan")
            print("self",self)
        ...
        local function onTouchMoved(touch, event)
            --cclog("CDLayerSZXXL::onTouchMoved")
        end
    
        local function onTouchEnded(touch, event)
            cclog("CDLayerSZXXL::onTouchEnded")
...
CDLayerSZXXL.m_pSimpleTouch = nil         -- 简单场按钮
CDLayerSZXXL.m_pNormalTouch = nil         -- 普通场按钮
CDLayerSZXXL.m_pHardTouch   = nil         -- 困难场按钮
CDLayerSZXXL.m_pChallengeTouch = nil      -- 挑战场按钮


CDLayerSZXXL.m_pSimpleGoldNode = nil      -- 简单场金币特效图标挂载点
CDLayerSZXXL.m_pNormalGoldNode = nil      -- 普通场金币特效图标挂载点
CDLayerSZXXL.m_pHardGoldNode   = nil      -- 困难场金币特效图标挂载点
CDLayerSZXXL.m_pChallengeGoldNode =nil    -- 挑战场金币特效图标挂载点


CDLayerSZXXL.m_pSimpleGoldLabel = nil     -- 简单场金币消耗文本显示框
CDLayerSZXXL.m_pNormalGoldLabel = nil     -- 普通场金币消耗文本显示框
CDLayerSZXXL.m_pHardGoldLabel   = nil     -- 困难场金币消耗文本显示框
CDLayerSZXXL.m_pChallengeGoldLabel = nil  -- 挑战场金币消耗文本显示框


CDLayerSZXXL.m_pGroupGame = nil

-- 初始化
function CDLayerSZXXL:initialMember()
    cclog("CDLayerSZXXL::initialMember")
...
function CDLayerSZXXL:releaseMember()
    cclog("CDLayerSZXXL::releaseMember")
...
-- 初始化
function CDLayerSZXXL:init()
    cclog("CDLayerSZXXL::init")
    self:setVisible(false)
    return true
end

-- 开启界面
function CDLayerSZXXL:open()
    cclog("CDLayerSZXXL::open")
    ...
    -- self:runAction( cc.Sequence:create( cc.DelayTime:create( 0.3), cc.CallFunc:create( CDLayerSZXXL.createRedTask)))
...
-- 添加所有节点
function CDLayerSZXXL:refreshInterface()
    cclog("CDLayerSZXXL::refreshInterface")
...
function CDLayerSZXXL:joinJYDSTable()
...
-- 关闭界面
function CDLayerSZXXL:close()
    cclog("CDLayerSZXXL::close")
...
-- 关闭界面
function CDLayerSZXXL:onClose()
    cclog("CDLayerSZXXL::onClose")
...
-----------------------------------------
-- 网络相关处理
function CDLayerSZXXL:Handle_Act_Ack( __event)
...
-----------------------------------------
-- ccb处理
function CDLayerSZXXL:onAssignCCBMemberVariable(loader)
    cclog("CDLayerSZXXL::onAssignCCBMemberVariable")
...
function CDLayerSZXXL:onResolveCCBCCControlSelector(loader)
    cclog("CDLayerSZXXL::onResolveCCBCCControlSelector")
    loader["onClose"] = function() self:onClose() end
end

-----------------------------------------
-- create
function CDLayerSZXXL.createCDLayer_SZXXL(pParent)
    cclog("CDLayerSZXXL::createCDLayerSZXXL")
    if  not pParent then
        return nil
    end
    local layer = CDLayerSZXXL.new()
    layer:init()
    pParent:addChild( layer)
    return layer
end

function CDLayerSZXXL:onLoadUI()
