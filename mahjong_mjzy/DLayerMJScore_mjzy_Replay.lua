...
/******************************************************
//Project:      ProjectX
//Moudle:       CDLayerMJScore_mjzy_Replay
...
-----------------------------------------
-- 类定义
CDLayerMJScore_mjzy_Replay = class("CDLayerMJScore_mjzy_Replay", CDAniLayerBase)
CDLayerMJScore_mjzy_Replay.__index = CDLayerMJScore_mjzy_Replay

-- 构造函数
function CDLayerMJScore_mjzy_Replay:ctor()
    cclog("CDLayerMJScore_mjzy_Replay::ctor")
    CDLayerMJScore_mjzy_Replay.super.ctor(self)
    CDLayerMJScore_mjzy_Replay.initialMember(self)

    local function onNodeEvent(event)
        if "exit" == event then
            CDLayerMJScore_mjzy_Replay.onExit(self)
        end
    end
    self:registerScriptHandler(onNodeEvent)
end

function CDLayerMJScore_mjzy_Replay:onExit()
    cclog("CDLayerMJScore_mjzy_Replay::onExit")
    self:stopAllActions()
    self:enableTouch(false)
    --模拟析构自身
    --uitest CDLayerMJScore_mjzy_Replay.releaseMember(self)
...
-----------------------------------------
-- 成员变量定义
CDLayerMJScore_mjzy_Replay.m_pButReturn = nil     -- 返回大厅
CDLayerMJScore_mjzy_Replay.m_pTxtReturn = nil

CDLayerMJScore_mjzy_Replay.m_pButShare = nil      -- 分享
CDLayerMJScore_mjzy_Replay.m_pTxtShare = nil

CDLayerMJScore_mjzy_Replay.m_pButAgain = nil      -- 再来一局
CDLayerMJScore_mjzy_Replay.m_pTxtAgain = nil

CDLayerMJScore_mjzy_Replay.m_pPlayerGroup = nil   -- 玩家组

CDLayerMJScore_mjzy_Replay.m_pDemo_lz = nil       -- 赖子容器
CDLayerMJScore_mjzy_Replay.m_pDemo_sc = nil       -- 胜负容器
CDLayerMJScore_mjzy_Replay.m_pTime = nil          -- 时间显示
CDLayerMJScore_mjzy_Replay.m_pBase = nil          -- 底注

CDLayerMJScore_mjzy_Replay.m_nWaitTime = 0        -- 等待时间
CDLayerMJScore_mjzy_Replay.m_uServerTime = 0      -- 服务器时间

CDLayerMJScore_mjzy_Replay.isSelfRoom = false    -- 是否是自建房
-----------------------------------------
-- 初始化
function CDLayerMJScore_mjzy_Replay:initialMember()
    cclog("CDLayerMJScore_mjzy_Replay::initialMember")
...
function CDLayerMJScore_mjzy_Replay:releaseMember()
    cclog("CDLayerMJScore_mjzy_Replay::releaseMember")
...
-- 初始化
function CDLayerMJScore_mjzy_Replay:init()
    cclog("CDLayerMJScore_mjzy_Replay::init")
...
-- 设置点击标志的开启和关闭
function CDLayerMJScore_mjzy_Replay:enableTouch( bEnable)
    ...
    if bEnable then
        ...
        local function onTouchBegan(touch, event)
            ...
            --    return false
            --end
            cclog("CDLayerMJScore_mjzy_Replay::onTouchBegan")
            return true
        end

        local function onTouchMoved(touch, event)
            --cclog("CDLayerMJScore_mjzy_Replay::onTouchMoved")
        end

        local function onTouchEnded(touch, event)
            cclog("CDLayerMJScore_mjzy_Replay::onTouchEnded")
        end
...
-- 添加一张牌
function CDLayerMJScore_mjzy_Replay:addMahjongToDemo( demo, pos, mahjong, lai_zi)
...
function CDLayerMJScore_mjzy_Replay:getNameById( table_data,id )
...
-- 改变对象编号根据玩家数
function CDLayerMJScore_mjzy_Replay:changeOrder( count, idx)
...
-- 设置界面数据
function CDLayerMJScore_mjzy_Replay:refreshInterface( data,table_Data,mahjong_math, lord_id,table_mjzy)
    ...
    for i = 1, count do
        ...
        -- 信息显示
        local curNickname,curChannel_nickname = self:getNameById(table_Data,player_score.player_id)
        if  curNickname then
            dtSetNickname( self.m_pPlayerGroup[order_idx].name, curNickname, curChannel_nickname)
        end
        ...
        if  player_score.player_id == lord_id then
            self.m_pPlayerGroup[order_idx].ico_zhuang:setVisible( true)
        else
            self.m_pPlayerGroup[order_idx].ico_zhuang:setVisible( false)
        end

        if  player_score.player_id == casinoclient.getInstance():getPlayerData():getId() then
        ...
        -- 风牌显示
        local curIndex = table_mjzy:changeOrder( table_mjzy:getPlayerIndex( player_score.player_id))
        local curHuaArray = table_mjzy.m_pPlayAI[curIndex]:getFangFMahjong()
        mahjong_math:defMahjongSort_stb(curHuaArray)
        print("风牌显示")
        dumpArray(curHuaArray)
        ...
        if  player_score.hupai_card > 0 then
            ...
            local bHu, mahjongs,curType = mahjong_math:canHuPai_defEX( curcards)
...
function CDLayerMJScore_mjzy_Replay:open( data,table_Data,mahjong_math, lord_id,table_mjzy)
    ...
    self:refreshInterface( data, table_Data,mahjong_math, lord_id,table_mjzy)
...
-- 关闭界面
function CDLayerMJScore_mjzy_Replay:close()
...
-- 返回大厅
function CDLayerMJScore_mjzy_Replay:onReturn()

end

-- 分享
function CDLayerMJScore_mjzy_Replay:onShare()

end

-- 再来一局
function CDLayerMJScore_mjzy_Replay:onAgain()
  
end

-- 动画播放完的回调处理
function CDLayerMJScore_mjzy_Replay:completedAnimationSequenceNamed(name)
...
function CDLayerMJScore_mjzy_Replay:onAssignCCBMemberVariable(loader)
    cclog("CDLayerMJScore_mjzy_Replay::onAssignCCBMemberVariable")
...
function CDLayerMJScore_mjzy_Replay:onResolveCCBCCControlSelector(loader)
...
-----------------------------------------
-- create
function CDLayerMJScore_mjzy_Replay.createCDLayerMJScore_mjzy_Replay(pParent)
    cclog("CDLayerMJScore_mjzy_Replay::createCDLayerMJScore_mjzy_Replay")
    if not pParent then
        return nil
    end
    local layer = CDLayerMJScore_mjzy_Replay.new()
    layer:init()
...
function CDLayerMJScore_mjzy_Replay:onLoadUI()
