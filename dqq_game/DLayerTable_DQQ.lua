...
function CDLayerTable_DQQ:initialMember()
    ...
    self.m_pNodeMode       = nil
    self.m_pJiZhongMode    = nil
    self.m_pWeiJiZhongMode = nil
    self.m_pLiangMode = {}
    self.m_pHuiMode = {}

    for i =1 ,2 do
        self.m_pLiangMode[i] = nil
        self.m_pHuiMode[i] = nil
    end
...
------------------------------------
    ...
    self.m_nRecordWeiJiZhong = 0         -- 记录未击中数量
...
function CDLayerTable_DQQ:Handle_Dqq_StartPlay(__event)
    ...
    if self.m_nFlag == 4 or self.m_nFlag == 5 then
        self.m_pSkillBar:setVisible(true)
    else
        self.m_pSkillBar:setVisible(false)
    end

    self.m_nTouchSkill = 0
    self.m_nRecordWeiJiZhong = 0
    ...
    local function  isWinOrLose()
        ...
        local count  = 0
        for i,v in pairs(self.m_arrBalloon) do
            if self.m_nFlag == 4 then
                ...
                if v.balItem.isBeyondPos then
                    table.remove(self.m_arrBalloon,i)
                    if v.balItem.m_nColor ~=self.m_nRand_Balloon then
                        count=count+1
                        if count >=3 then
                            count= 3
                            break
        ...
        self.m_nRecordWeiJiZhong = self.m_nRecordWeiJiZhong+self.m_nWeiJiZhong +count
        if self.m_nRecordWeiJiZhong >=3 then 
            self.m_nRecordWeiJiZhong =3 
        end

        self.m_pWeiJiZhong:setString(self.m_nRecordWeiJiZhong.."/"..3)
    ...
    local function TimerStart()
        ...
        if self.m_pttf then
            self.m_pttf:setString(string.format("计时:%d",self.m_nTime))
        end
    end 
...
function CDLayerTable_DQQ:showLastView()
    ...
    if self.m_nFlag == 5 then
        self.m_pNodeMode:setVisible(false)
        ...
        self.m_pNodeMode:setVisible(true)

        self.m_pJiZhongMode:setString(self.m_nAlraadyDqq.."/"..self.m_nNeedDqq)
        self.m_pWeiJiZhongMode:setString(self.m_nRecordWeiJiZhong.."/"..3)
    end
    ...
    if self.isWin then
        ...
        if self.m_pNodeMode5:isVisible() then
            for i = 1 ,3 do
        ...
        if self.m_pNodeMode:isVisible() then
            for i = 1 ,2 do
                self.m_pLiangMode[i]:setVisible(true)
                self.m_pHuiMode[i]:setVisible(false)
            end
        end
        ...
        if self.m_pNodeMode:isVisible() then
            for i = 1 ,2 do
                self.m_pLiangMode[i]:setVisible(false)
                self.m_pHuiMode[i]:setVisible(true)
            end
        end
...
function CDLayerTable_DQQ:createUserInterface(flag)
    ...
    -- --加载plist资源文件，不能在这里读取，资源太多，加载不完
    -- cc.SpriteFrameCache:getInstance():addSpriteFrames("Game_DQQ.plist", "Game_DQQ.png",true)
    ...
    if self.m_nFlag == 4 then
        ...
        self.m_pSpriteWarnning = cc.Sprite:createWithSpriteFrameName("prompt_"..self.m_nRand_Balloon..".png")
...
function CDLayerTable_DQQ:preCreate()
    ...
    if not self.isAgainGame then --再来一局无需重新创建桌子
        ...
        for i = 1 ,self.m_nQiQiuZhu do
            local  BalPicture = cc.Sprite:createWithSpriteFrameName(string.format("zhuzi.png"))
...
function CDLayerTable_DQQ:onTip()
    -- local tipStr = "宋时江天天到厕所吃狗屎"

    -- local function sendJoinJYDS()
        
    -- end 
    -- g_pSceneTable.m_pLayerPromptDialog:open(tipStr,cc.CallFunc:create(sendJoinJYDS),10)

    if self.m_pSkillBar:isVisible() then
        self.m_nTouchSkill = self.m_nTouchSkill + 1

        self:skillNeedGold(5000,self.m_nTouchSkill)
        print("技能的点击")
        if self.m_nFlag == 4 then
            if self.m_nRecordWeiJiZhong >0 then
                self.m_nRecordWeiJiZhong = self.m_nRecordWeiJiZhong - 1
                self.m_pWeiJiZhong:setString(self.m_nRecordWeiJiZhong.."/"..3)
            end

        elseif self.m_nFlag == 5 then
            self.m_nTime = self.m_nTime +6 
        end
    end
end


function CDLayerTable_DQQ:skillNeedGold(gold,count)
    if casinoclient.getInstance():getPlayerData():getPlayerResourceGold() < (gold*count) then
        return 
    end
...
function CDLayerTable_DQQ:onAssignCCBMemberVariable(loader)
    ...
    self.m_pNodeMode     = loader["Mode_showNode"]
    self.m_pJiZhongMode  = loader["mode_jizhong"]
    self.m_pWeiJiZhongMode = loader["mode_weijizhong"]

    for i =1 ,2 do
        self.m_pLiangMode[i] = loader["Mode_liang"..i]
        self.m_pHuiMode[i] = loader["Mode_hui"..i]
    end

    -- 技能相关绑定
    self.m_pSkillBar        = loader["skill_bar"]               -- 技能根Node
    self.m_pOnTipBtn        = loader["but_tip"]                 -- 技能：提示按钮
    --self.m_pOnResetBtn      = loader["but_reset"]               -- 技能：重置按钮
    self.m_pTipExpLabel     = loader["tip_expend_gold"]        -- 提示技能消耗文本
    --self.m_pResetExpLabel   = loader["reset_expend_gold"]       -- 重置技能消耗文本
