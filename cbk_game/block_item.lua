...
function CDBlockItem:setLabelColor(color)
    if self.m_pLabel then
        self.m_pLabel:setTextColor(color)
    end
end
...
function CDBlockItem:moveDownAndCleanUp(num)
    ...
    self:runAction(cc.Sequence:create(cc.MoveTo:create( 0.1, cc.p(self:getPositionX()+finalScreen.width/num,self:getPositionY())),cc.DelayTime:create( 0.15), cc.CallFunc:create( detailClearOtherBlock)))
