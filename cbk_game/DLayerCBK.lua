...
-----------------------------------------
-- 类定义
CDLayerCBK = class("CDLayerCBK", CDTKDTableView)
CDLayerCBK.__index = CDLayerCBK

-- 构造函数
function CDLayerCBK:ctor()
    cclog("CDLayerCBK::ctor")
    CDLayerCBK.super.ctor(self)
    CDLayerCBK.initialMember(self)
...
function CDLayerCBK:onEnter( ... )

	cclog("CDLayerCBK::onEnter")
	-- dtOpenWaiting(self)

end

function CDLayerCBK:onExit()
    cclog("CDLayerCBK::onExit")

    self:stopAllActions()
    self:enableTouch(false)
    --模拟析构自身
    CDLayerCBK.releaseMember(self)
...
-- 设置点击标志的开启和关闭
function CDLayerCBK:enableTouch( bEnable)
	cclog("CDLayerCBK::enableTouch")
    print("===================")
	...
	if bEnable then
        ...
        local function onTouchMoved(touch, event)
            --cclog("CDLayerCBK::onTouchMoved")
        end
    
        local function onTouchEnded(touch, event)
            cclog("CDLayerCBK::onTouchEnded")
...
CDLayerCBK.m_pSimpleTouch = nil         -- 简单场按钮
CDLayerCBK.m_pNormalTouch = nil         -- 普通场按钮
CDLayerCBK.m_pHardTouch   = nil         -- 困难场按钮
CDLayerCBK.m_pChallengeTouch = nil      -- 挑战场按钮


CDLayerCBK.m_pSimpleGoldNode = nil      -- 简单场金币特效图标挂载点
CDLayerCBK.m_pNormalGoldNode = nil      -- 普通场金币特效图标挂载点
CDLayerCBK.m_pHardGoldNode   = nil      -- 困难场金币特效图标挂载点
CDLayerCBK.m_pChallengeGoldNode =nil    -- 挑战场金币特效图标挂载点

CDLayerCBK.m_pSimpleGoldLabel = nil     -- 简单场金币消耗文本显示框
CDLayerCBK.m_pNormalGoldLabel = nil     -- 普通场金币消耗文本显示框
CDLayerCBK.m_pHardGoldLabel   = nil     -- 困难场金币消耗文本显示框
CDLayerCBK.m_pChallengeGoldLabel = nil  -- 挑战场金币消耗文本显示框


CDLayerCBK.m_pGroupGame = nil

-- 初始化
function CDLayerCBK:initialMember()
    cclog("CDLayerCBK::initialMember")
...
function CDLayerCBK:releaseMember()
    cclog("CDLayerCBK::releaseMember")
...
-- 初始化
function CDLayerCBK:init()
    cclog("CDLayerCBK::init")
    self:setVisible(false)
    return true
end

-- 开启界面
function CDLayerCBK:open()
    cclog("CDLayerCBK::open")
    ...
    -- self:runAction( cc.Sequence:create( cc.DelayTime:create( 0.3), cc.CallFunc:create( CDLayerCBK.createRedTask)))
...
-- 添加所有节点
function CDLayerCBK:refreshInterface()
    cclog("CDLayerCBK::refreshInterface")
...
function CDLayerCBK:joinDqqTable()
    ...
    --         tipStr = string.format("确定花费%d%s开始麻将连连看游戏吗？", self.m_nNeedGold, "金币")
            ...
            --踩白块
            g_pSceneHall:goToScenneTable_CBK(self.m_nFlag)
...
-- 关闭界面
function CDLayerCBK:close()
    cclog("CDLayerCBK::close")
...
-- 关闭界面
function CDLayerCBK:onClose()
    cclog("CDLayerCBK::onClose")
...
-----------------------------------------
-- 网络相关处理
function CDLayerCBK:Handle_Act_Ack( __event)
...
-----------------------------------------
-- ccb处理
function CDLayerCBK:onAssignCCBMemberVariable(loader)
    cclog("CDLayerCBK::onAssignCCBMemberVariable")
...
function CDLayerCBK:onResolveCCBCCControlSelector(loader)
    cclog("CDLayerCBK::onResolveCCBCCControlSelector")
    loader["onClose"] = function() self:onClose() end
end

-----------------------------------------
-- create
function CDLayerCBK.createCDLayerCBK(pParent)
    cclog("CDLayerCBK::createCDLayerCBK")
    if  not pParent then
        return nil
    end
    local layer = CDLayerCBK.new()
    layer:init()
    pParent:addChild( layer)
    return layer
end

function CDLayerCBK:onLoadUI()
    ...
    local node  = CCBReaderLoad("CDLayerCBK.ccbi",proxy,loader)
