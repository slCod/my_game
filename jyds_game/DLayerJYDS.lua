...
function CDLayerJYDS:enableTouch( bEnable)
	...
	if bEnable then
        ...
        local function onTouchEnded(touch, event)
            ...
            sRect = self.m_pTianJiangTouch:getBoundingBox()
            if cc.rectContainsPoint(sRect,sJudge) then
                self.m_nFlag = 5
                self.m_nNeedGold = 5000
                self:joinJYDSTable()
...
CDLayerJYDS.m_pTianJiangTouch = nil      -- 填将场按钮
...
CDLayerJYDS.m_pChallengeGoldNode = nil   -- 填将场金币特效图标挂载点
...
CDLayerJYDS.m_pTianJiangGoldLabel = nil  -- 填将场金币消耗文本显示框
...
function CDLayerJYDS:refreshInterface()
    ...
    if not self.m_pTianJiangGoldNode:getChildByTag(0) then
        local eff_gold5 = CDCCBAniObject.createCCBAniObject( self.m_pTianJiangGoldNode, "x_tx_gold.ccbi", pos, 0)
        eff_gold5:setTag(0)
    ...
    self.m_pTianJiangGoldLabel:setString("5000")
end
...
function CDLayerJYDS:onAssignCCBMemberVariable(loader)
    ...
    self.m_pTianJiangTouch = loader["touch_tianjiang"]

    -- 简单、普通、困难 挑战 填将 金币特效图片挂载点
    ...
    self.m_pTianJiangGoldNode = loader["tianjiang_gold_node"]

    -- 简单、普通、困难 挑战 填将 金币消耗文本显示控件
    ...
    self.m_pTianJiangGoldLabel = loader["label_tianjiang"]
