...
function CDLayerTable_JYDS:initialMember()
    ...
    -- self.m_pBtnGoToHall   = nil
     ...
     self.m_pBtnChaKan = nil
     self.m_pNodeChaKan = nil
------------------------------------
...
function CDLayerTable_JYDS:IntoGameStage()
    if self.m_nStage ==  0 then
        ...
        local function showMahjong()
            if self.m_nFlag ~= 4 then
                ...
                self.m_pCurMahIndex:setVisible(true)
                local function showMahjongDelay()
                    if self.m_pShowMah[index-1] then
                        self.m_pShowMah[index-1]:setVisible(false)
                        self.m_pCurMahIndex:setString("")
                    end
                end 
                self:runAction(cc.Sequence:create(cc.DelayTime:create(0.5),cc.CallFunc:create(showMahjongDelay),cc.DelayTime:create(0.3)))
            end
            ...
            if self.m_pCurMahIndex then
                self.m_pCurMahIndex:setString(string.format("第%d张麻将",index))
            end
            --挑战模式下无需显示
            if self.m_nFlag ~= 4 then
        ...
        if self.m_nFlag ~= 5 then
            ...
            if not self.schedulerID1 then
                self.schedulerID1 = cc.Director:getInstance():getScheduler():scheduleScriptFunc(showMahjong,time,false)
            end
        else
    ...
    elseif   self.m_nStage ==  2 then
        ...
        self.m_ntimeCount = self.m_nNeedTime

        local function startGame ()
            print("self.m_ntimeCount------>",self.m_ntimeCount)
            self.m_ntimeCount = self.m_ntimeCount-1
            if self.m_ntimeCount<0 then
                self.m_ntimeCount =0
                self:cancelSchedule2()
                self.isWin = false
                self:showLastView()
            ...
            self.m_pTimeLabel:setString(self.m_ntimeCount)
        end 
...
function CDLayerTable_JYDS:showLastView()
    print("CDLayerTable_JYDS:showLastView()")
    --DEF_GAME_ISEND =true
    ...
    self.m_pBtnChaKan:setVisible(false)
    ...
    if self.isWin then
        ...
        self.m_PEndSprite = cc.Sprite:createWithSpriteFrameName(tempSprite)
        self.m_pWinNode:addChild(self.m_PEndSprite)
        self.m_PEndSprite:setPosition(cc.p(145,205))
        ...
        if self.m_nFlag < 4 then
            self.m_pBtnChaKan:setVisible(true)
        end
    ...
    if self.m_pLabelNum_GuanShu then
        self.m_pLabelNum_GuanShu:setString(string.format("%d",self.m_nTotalRound))
    end

    if self.m_pLabelUse_Time then
        self.m_pLabelUse_Time:setString(string.format("%d",self.m_nNeedTime - self.m_ntimeCount))
    end
...
function CDLayerTable_JYDS:createAllMahjongBtnAndMode(flag)
    ...
    if self.m_nFlag == 4 then
        ...
        if self.m_pCurMahIndex == nil then
            self.m_pCurMahIndex = cc.LabelTTF:create("0.000","Courier New",60)
            self.m_pCurMahIndex:setColor(cc.c3b(255,0,0))
            self.m_pCurMahIndex:setVisible(false)
            self.m_pCurMahIndex:setPosition(cc.p(ScreenSize.width/2,ScreenSize.height/2+170))
            self.m_pMahjongShowLayer:addChild(self.m_pCurMahIndex)
        end 
        ...
        if v.item:touchInFromPoint(point) then
            ...
            elseif self.m_nFlag == 5 then
                ...
                if v.value == self.m_nRandMahjongArr[self.m_nGuessIndex] then
                    ...
                    -- 失败了
                    --点击错误时出现错误图标
                    if self.m_pShowFloorMah[self.m_nGuessIndex] then
                        self.m_pShowFloorMah[self.m_nGuessIndex]:showErrorMah_JYDS()
                    end
                    self.isWin = false
                    self:showLastView()
                    self:cancelSchedule2()
...
-- 使用技能的接口
function CDLayerTable_JYDS:useSkill(num_skill,gold)
    self.m_bInTheGame = false
    if num_skill == 1 or num_skill == 2 then
        --麻将属于哪一类
        if self.m_nFlag == 4 then
        ...
        elseif self.m_nFlag == 5 then
            ...
            --self.m_nRandMahjongArr[self.m_nGuessIndex]


        end

    elseif num_skill == 3 then
        --点一次增加5s的时间
    end
end
...
---查看
function CDLayerTable_JYDS:onChaKan()
  
    if self.m_pLastViewLayer:isVisible() and self.m_pBtnChaKan:isVisible() then
        if not self.m_bChaKan then
            self.m_pNodeChaKan:setVisible(true)
            for i = 1 ,TABLE_SIZE(self.m_nRandMahjongArr) do
    
                local mahjong= CDMahjong.createCDMahjong(self.m_pNodeChaKan)
                mahjong:setMahjongScale(1.2)
                mahjong:initMahjongWithFile( string.format( "t_%u.png", self.m_nRandMahjongArr[i]))
                mahjong:setPosition(cc.p(100+mahjong.m_nSizeW*i,0))
            end
        else
            if self.m_pNodeChaKan then
                self.m_pNodeChaKan:setVisible(false)
            end
        end

        self.m_bChaKan = not self.m_bChaKan
    end
end
...
function CDLayerTable_JYDS:onAssignCCBMemberVariable(loader)
    ...
    self.m_pBtnChaKan     = loader["button_chakan"]
    self.m_pNodeChaKan    =  loader["Node_ChaKan"]

    -- 
    self.m_pLabelNum_GuanShu = loader["num_guanshu"]

    self.m_pLabelUse_Time   = loader["num_usetime"]
...
function CDLayerTable_JYDS:onResolveCCBCCControlSelector(loader)
    ...
    loader["onChaKan"]      = function() self:onChaKan()   end   
